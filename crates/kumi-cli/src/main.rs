...
use kumi_diagnostics::{Diagnostic, DiagnosticPrinter, Severity};
use std::fs;
...
fn run_parser_test(filename: &str) {
    ...
    if !lex_errors.is_empty() {
        ...
        if lex_errors.iter().any(|e| e.severity == Severity::Error) {
            std::process::exit(1);
        }
    }
    ...
    let parser = kumi_syntax::parse::Parser::new(&tokens, &source);
    ...
    if !ast.errors.is_empty() {
        ...
        has_errors |= ast.errors.iter().any(|e| e.severity == Severity::Error);
    }

    let mem_after_parse = get_peak_memory_mb();

    let check_start = Instant::now();
    let checker = kumi_semantics::Checker::new();
    ...
    if !check_errors.is_empty() {
        ...
        has_errors |= check_errors.iter().any(|e| e.severity == Severity::Error);
    }
...
fn main() {
    ...
    macro_rules! check_help {
        ($args:expr, $name:literal) => {
            if $args.help {
                args::print_subcommand_help($name);
                return;
            }
        };
    }
    ...
    match cli.command.unwrap() {
        ...
        Commands::Add(args) => {
            ...
            if let Some(name) = args.name {
                ...
                args::print_subcommand_help("add");
                std::process::exit(1);
            }
        }
        ...
        Commands::Remove(args) => {
            ...
            if let Some(name) = args.name {
                ...
                eprintln!("Error: missing required argument <name>");
                args::print_subcommand_help("remove");
                std::process::exit(1);
            }
        }
        Commands::Fmt(args) => {
        ...
        Commands::Search(args) => {
            ...
            if let Some(query) = args.query {
                ...
                args::print_subcommand_help("search");
                std::process::exit(1);
            }
        }
    }
